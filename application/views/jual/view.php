	...
	<section class="content-header">
		...
		<?php if($info['cek'] =='0'){ ?>
			<?php if($info['jurnal_id'] ==''){ ?>
				...
				<a href="<?php echo site_url('jual/jurnal/'.$info['id']);?>" class="btn btn-info" ><i class='fa fa-check-square-o'></i> Selesai/ACC</a>
				<?php } ?>
			<?php }else{ ?>
			...
			<a href="<?php echo site_url('jual/rounding/'.$info['id']);?>" class="btn btn-success" ><i class='fa fa-edit'></i> Edit</a>
			<?php }else{ ?>
				<?php if($this->session->userdata('ADMIN')<='3'){ ?>
					<?php if($info['status'] <'3'){ ?>
					<a target="_blank" href="<?php echo site_url('jual/add_pay/'.$info['id']);?>" class="btn btn-success" ><i class='fa fa-plus'></i>  Tambah Pelunasan</a>
					<?php }else{ ?>
					<a target="_blank" href="<?php echo site_url('jual/view_pay/'.$info['id']);?>" class="btn btn-primary" ><i class='fa fa-file'></i>  Lihat Pelunasan</a>
					<?php } ?>
				<?php } ?>
			<?php } ?>

				...
				<div class="box">
					<div class="box-header">
						<div class="btn-group">
							...
							<a class="btn btn-primary" href="<?php echo site_url();?>beli/wait"> <?php echo $this->bbm_model->JmlBeliWaiting();?> Draft</a>
							<a class="btn btn-primary" href="<?php echo site_url();?>beli/prosses"> <?php echo $this->bbm_model->JmlBeliProsses();?> Proses ACC</a>
							<a class="btn btn-primary" href="<?php echo site_url();?>beli/success"> <?php echo $this->bbm_model->JmlBeliSuccess();?> Sudah ACC</a>
						<?php if($this->session->userdata('ADMIN') <='3'){ ?>
							<a class="btn btn-primary" href="<?php echo site_url();?>beli/pay"> <?php echo $this->bbm_model->JmlBeliPay();?> Proses Bayar</a>
							<a class="btn btn-primary" href="<?php echo site_url();?>beli/paid"> <?php echo $this->bbm_model->JmlBeliPaid();?> Sudah Bayar</a>
						<?php } ?>
							<a class="btn btn-danger" href="<?php echo site_url();?>beli/cancel"> <?php echo $this->bbm_model->JmlBeliCancel();?> Cancel</a>
						</div>
					</div><!-- /.box-header -->
					<div class="box-body">
											...
											<div class="btn-group-vertical">
												...
												<?php if($row->status =='2'){ ?>
													<?php if($this->session->userdata('ADMIN')<'4'){ ?>
														<a title="Tambah Pelunasan" href="<?php echo site_url('beli/add_pay/'.$row->id);?>" class="btn btn-danger btn-xs" ><i class='fa fa-money'></i> Bayar Pelunasan</a>
													<?php } ?>
												<?php } ?>
												<?php if($row->status >='3'){ ?>
													<a title="Lihat Bukti" href="<?php echo site_url('beli/view_pay/'.$row->id);?>" class="btn btn-success btn-xs" ><i class='fa fa-file'></i> Lihat Bukti</a>
												<?php } ?>
												<br>
												<?php if($this->bbm_model->JmlSisaBayarBeli($row->id) =='0'){ ?>
												<a class="btn btn-default btn-xs" ><b>LUNAS</b></a><?php }else{ ?>
												<a class="btn btn-default btn-xs" ><b>Belum Dilunasi : </b><?php echo number_format($this->bbm_model->JmlSisaBayarBeli($row->id), 0, ',', '.');?></a>
													<?php if($this->session->userdata('ADMIN')<'4'){ ?>
														<?php if($this->bbm_model->JmlBayarBeli($row->id) =='0'){ ?>
															<a class="btn btn-default btn-xs" ><b>Pelunasan Menunggu ACC</b></a>
														<?php }else{ ?>
															<?php if($row->status >='3'){ ?>
																<a title="Tambah Pelunasan" href="<?php echo site_url('beli/add_pay/'.$row->id);?>" class="btn btn-danger btn-xs" ><i class='fa fa-money'></i> Bayar Pelunasan</a>
															<?php } ?>

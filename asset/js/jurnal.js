...
function addRow() {
	...
	if (emptyrows == 0 ) {
		var ddlAkun = '<select name="akun[]" id="akun'+lastRow+'" class="combo form-control" >';
		ddlAkun += $("#akun1").html();
		ddlAkun += '</select>';
		var txtDebit = '<input name="debit'+lastRow+'" id="debit'+lastRow+'" class="field form-control" title="Debit harus diisi dengan angka" onBlur="cekDebit('+lastRow+');" />';
		var txtKredit = '<input name="kredit'+lastRow+'" id="kredit'+lastRow+'" class="field form-control" title="Kredit harus diisi dengan angka" onBlur="cekKredit('+lastRow+');" />';
		...
		oDialog.html("Silahkan mengisi data pada baris yang tersedia terlebih dahulu, sebelum menambah baris diatas.");
		oDialog.dialog('open');
	}
}

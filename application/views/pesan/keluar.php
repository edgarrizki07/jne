...
<?php $boutbox = $this->m_pesan->JmlBOutbox($this->session->userdata('SESS_USER_ID'));?>
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Send
            <small><?php echo $boutbox;?>  unread messages</small>
          </h1>
          <ol class="breadcrumb">
                ...
                <div class="box-header with-border">
                  ...
                  <div class="box-tools pull-right">
                    <div class="has-feedback">
                      Showing 1-<?php echo $outbox;?> / Unread <?php echo $boutbox;?> / All <?php echo $outbox;?>
                    </div>
                  </div><!-- /.box-tools -->
                </div><!-- /.box-header -->
                <div class="box-body no-padding">
                  <div class="table-responsive mailbox-messages">
                    <table class="table table-hover table-striped">
                      ...
                      <tbody>
					<?php $this->db->order_by('id', 'desc'); $mail = $this->db->get_where('pesan',array('cekkepada'=>'0','dari'=>$this->session->userdata('SESS_USER_ID'))); $text = $mail->result(); $no=0; foreach($text as $row ): $no++;?>
					...
					<?php } ?>
                          ...
                          <td width="170" class="mailbox-name"><a href="<?php echo site_url('pesan/lihat/'.$row->id);?>"><?php echo $this->user_model->NamaUser($row->kepada);?> <?php echo $this->user_model->NamaBUser($row->kepada);?></a></td>
						  ...
						  <td width="20" class="mailbox-star" width="50" align="center"><a href="<?php echo base_url();?>pesan/hapus_outbox/<?php echo $row->id;?>" class="btn btn-default btn-xs" onClick="return confirm('Anda yakin ingin menghapus pesan ini?')"> <i class='fa fa-trash-o'></i></a></td>
                        </tr>
					<?php endforeach;?>
                      </tbody>
                    </table><!-- /.table -->

...
class Laporan_keuangan extends CI_Controller {
	...
	function laporan_laba_rugi_excel()
	{
		$id=$this->session->userdata('SESS_WP_ID');
		$data['wajib_pajak_data'] = $this->pajak_model->get_data($id);	
		$bulan = $this->uri->segment(3);
		...
		$data['tahun'] = $this->uri->segment(4);
		$data['laba_rugi_data'] = $this->_get_laba_rugi_data($bulan,$data['tahun']);
		$this->load->view('laporan_keuangan/laba_rugi_excel', $data);
	}
...
*/
	...
	function laporan_neraca_excel()
	{
		...
		$data['tahun'] = $this->uri->segment(4);
		$data['neraca_data'] = $this->_get_neraca_data($bulan,$data['tahun']);
		$this->load->view('laporan_keuangan/neraca_excel', $data);
	}

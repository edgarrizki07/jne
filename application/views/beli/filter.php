...
<?php if($info->num_rows()>0){
	...
	<tbody>
		<tr>
			<td><?php echo $no;?></td>
			<td align="center"><?php echo $this->app_model->tgl_str($db['tgl']);?></br><a  href="<?php echo site_url('beli/view/'.$db['id']);?>"><?php echo $db['jurnal_id'];?>/PO/<?php echo $this->pajak_model->KodeCabang($db['wp_id']);?>/<?php echo $db['id'];?></a></br><?php if($db['status'] =='2'){ ?>Success<?php } else if($db['status'] =='3'){ ?>Pay Prosses<?php } else if($db['status'] =='4'){ ?>Paid<?php } ?></td>
		<?php if($this->session->userdata('ADMIN') =='1'){ ?>
			<td><?php echo $this->pajak_model->KotaCabang($db['wp_id']);?></br><?php echo $this->user_model->NamaUser($db['login_id']);?></td>
		<?php } ?>
			<td><b><?php echo $this->supplier_model->CPSupplier($db['supplier_id']);?></b></br><?php echo $this->supplier_model->NamaSupplier($db['supplier_id']);?></td>
			...
			<td>PPN <a class="pull-right"><?php echo number_format ($db['tot4'], 0, ',', '.');?></a></br>PPh <a class="pull-right"><?php echo number_format ($db['tot6'], 0, ',', '.');?></a></br>PBBKB <a class="pull-right"><?php echo number_format ($db['tot5'], 0, ',', '.');?></a></td>
...
<?php if($g_total=='0'){ ?><?php }else{ ?>
    <tfoot>
        <tr>
			...
			<th><b class="pull-right"><?php echo number_format ($g_total3, 0, ',', '.');?></b></br><b class="pull-right"><?php echo number_format ($g_total5, 0, ',', '.');?></b></br><b class="pull-right"><?php echo number_format ($g_total4, 0, ',', '.');?></b></th>

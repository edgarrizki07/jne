                            ...
                            <div class="box box-warning">
                                <div class="box-header">
									<h3 class="box-title">
										...
										<b>Proses Bayar : </b><?php echo number_format($this->bbm_model->JmlPayJual($info['id']), 0, ',', '.');?>
										<b>Sudah Terbayar : </b><?php echo number_format($this->bbm_model->JmlPaidJual($info['id']), 0, ',', '.');?>
										<b>Belum Bayar : </b><?php echo number_format($this->bbm_model->JmlSisaBayarJual($info['id']), 0, ',', '.');?>
									</h3>
                                </div>
                                <div class="box-body">
								<?php if($this->bbm_model->JmlBayarJual($info['id']) =='0'){ ?>
									<h4>Jumlah Total Pembayaran telah Terpenuhi, Anda Tidak dapat input pembayaran lagi</h4>
									<a href="<?php echo site_url('jual/view_pay/'.$info['id']);?>" class="btn btn-info" ><i class="fa fa-eye"></i> Lihat Pembayaran</a>
									<?php }else{ ?>
									<div class="hidden">
									...
									<div class="form-group">
										...
										<input type="hidden" name="id_jual" value="<?php echo $this->uri->segment(3);?>" class="form-control"/>
										<input type="text" name="tglbyr" id="datepicker"  value="<?php echo date('Y-m-d');?>" class="form-control" required/>
										</div>
									</div>
									<?php $t = "SELECT sum(jmlbyr) as jml FROM jual_bayar WHERE id_jual='$uri' "; $d = $this->db->query($t); $r = $d->num_rows(); ?>
									<?php if($r>0){ foreach($d->result() as $h){ $totbyr = $h->jml; } }else{ $totbyr = 0; } ?>
									<div class="form-group">
										<div class="col-xs-4">
											<label><strong>Diterima Sejumlah</strong></label>
										</div>
										<div class="input-group"><span class="input-group-btn"><span>
										<input type="text" name="jmlbyr" value="<?php echo $this->bbm_model->Total9Jual($this->uri->segment(3))-$totbyr;?>" class="form-control"/>
										</div>
									</div>
									<div class="form-group">
										<div class="col-xs-4">
											<label><strong>Masuk ke akun Bank</strong></label>
										</div>
										<div class="input-group"><span class="input-group-btn"><span>
									...
									<div class="form-group">
										...
										<div class="input-group"><span class="input-group-btn"><span>
											<textarea size="450" type="text" name="keterangan" class="form-control" required/>Diterima Pembayaran Invoice (<?php echo $this->bbm_model->JmlJual($this->uri->segment(3));?>L)</textarea>
										</div>
									</div>
									<div class="form-group">
										<div class="col-xs-4">

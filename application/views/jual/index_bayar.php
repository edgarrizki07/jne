<div class="content-wrapper">
	...
	<?php  $qty=$row->jml; $jml=$row->jml*$row->harga; $d=$row->discount; $disc=$jml*$d/100; $disctotal=$jml-$disc; $ppn=$disctotal/10*($row->ppn); $pbbkb=$disctotal*($row->npbbkb/100)*($row->pbbkb); $pph=$disctotal*3/1000*($row->pph); 
	$t=($row->ohp); $td=($t*$qty); $tt=($td)+($row->ppnohp*$td/10); $gr_total=$disctotal+$ppn+$pbbkb+$pph+($tt); ?>
								<tr>
									<td><?php echo $no;?></td>
									<td>
										<div class="btn-group">
										  ...
										  <ul class="dropdown-menu" role="menu">
											...
											<?php }else{ ?>
												<?php if($row->status =='0'){ ?>
													<?php if( $row->login_id==$this->session->userdata('SESS_USER_ID')){ ?>
														<?php if($row->cek =='0'){ ?>
														<a title="Edit PO" href="<?php echo site_url('jual/rounding/'.$row->id);?>" class="btn btn-success btn-xs" ><i class='fa fa-file'></i> Edit Invoice </a>
														<?php }else{ ?>
											...
											<?php if($row->status =='2'){ ?>
												<?php if($this->session->userdata('ADMIN')<'4'){ ?>
													<a title="Tambah Bayar" href="<?php echo site_url('jual/add_pay/'.$row->id);?>" class="btn btn-danger btn-xs" ><i class='fa fa-money'></i> Terima Pembayaran</a>
												<?php } ?>
											<?php } ?>
											<br>
											<?php if($row->status >='3'){ ?>
												<a title="Lihat Bukti" href="<?php echo site_url('jual/view_pay/'.$row->id);?>" class="btn btn-success btn-xs" ><i class='fa fa-file'></i> Lihat Bukti</a>
											<?php } ?>
											
											<?php if($this->bbm_model->JmlSisaBayarJual($row->id) =='0'){ ?>
												<a class="btn btn-default btn-xs" ><b>LUNAS</b></a>
											<?php }else{ ?>
												<a class="btn btn-default btn-xs" ><b>Belum Bayar : </b><?php echo number_format($this->bbm_model->JmlSisaBayarJual($row->id), 0, ',', '.');?></a>
												<?php if($this->session->userdata('ADMIN')<'4'){ ?>
													<?php if($this->bbm_model->JmlBayarJual($row->id) =='0'){ ?>
														<a class="btn btn-default btn-xs" ><b>Pembayaran Menunggu ACC</b></a>
														<?php }else{ ?>
															<?php if($row->status >='3'){ ?>
																<a title="Tambah Bayar" href="<?php echo site_url('jual/add_pay/'.$row->id);?>" class="btn btn-info btn-xs" ><i class='fa fa-money'></i> Terima Pembayaran</a>
															<?php } ?>
													<?php } ?>
												<?php } ?>
											<?php } ?>
											...
											<?php if($this->session->userdata('ADMIN')=='1'){ ?>
												<div class="btn-group">
													<?php if($row->cek =='0'){ ?>
													<a title="cancel" href="<?php echo base_url();?>jual/hapus/<?php echo $row->id;?>" class="btn btn-danger btn-xs" 
									...
									<br><?php echo $this->jurnal_model->tgl_singkatan($row->tgl);?><br><a title="nomor urut penjualan dari masing-masing cabang"><?php echo $row->id_jual;?></a></td>
								<?php if($this->session->userdata('ADMIN') =='1'){ ?>

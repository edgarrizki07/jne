	...
	<section class="content-header">
		...
		<?php if($info['cek'] =='0'){ ?>
			<?php if($info['jurnal_id'] ==''){ ?>
				...
				<a href="<?php echo site_url('beli/jurnal/'.$info['id']);?>" class="btn btn-info" ><i class='fa fa-check-square-o'></i> Selesai/ACC</a>
				<?php } ?>
			<?php }else{ ?>
	<?php if($this->session->userdata('ADMIN')=='1'){ ?>
<a target="_blank" href="<?php echo site_url('beli/pdf/'.$info['id']);?>" class="btn btn-success" ><i class='fa fa-print'></i> Print</a>
	<?php } ?>
			...
			<a href="<?php echo site_url('beli/rounding/'.$info['id']);?>" class="btn btn-success" ><i class='fa fa-edit'></i> Edit</a>
			<?php }else{ ?>
				<?php if($this->session->userdata('ADMIN')<='3'){ ?>
					<?php if($info['status'] <'3'){ ?>
					<a target="_blank" href="<?php echo site_url('beli/add_pay/'.$info['id']);?>" class="btn btn-success" ><i class='fa fa-plus'></i>  Tambah Pelunasan</a>
					<?php }else{ ?>
					<a target="_blank" href="<?php echo site_url('beli/view_pay/'.$info['id']);?>" class="btn btn-primary" ><i class='fa fa-file'></i>  Lihat Pelunasan</a>
					<?php } ?>
				<?php } ?>
			<?php } ?>
	...
	<section class="invoice">
		<div class="row invoice-info">
			...
			<div class="col-sm-5">
				<table width='100%'>
					<tr>
						<td>No PO</td><td> : </td><td><?php echo $info['id_beli'];?>.<?php echo $this->jurnal_model->ambilTgl($info['tgl']);?>/PO/<?php echo $this->pajak_model->KodeCabang($info['wp_id']);?>/<?php echo $info['id'];?>/<?php echo $this->jurnal_model->getRomawi($this->jurnal_model->ambilBln($info['tgl']));?>/<?php echo $this->jurnal_model->ambilThn($info['tgl']);?></td>
					</tr>
					<tr>
						<td>Tanggal</td><td> : </td><td><?php echo $this->app_model->tgl_indo($info['tgl']);?></td>
					</tr>
					<tr>
						<td>Metode</td><td> : </td><td><strong><?php echo $info['term'];?></strong></td>
					</tr>
					<tr>
						<td>Hauler</td><td> : </td><td><strong><?php echo $this->setting_model->nama();?></strong></td>
					</tr>
					<tr>
						<td>SH</td><td> : </td><td><strong><?php echo $info['sh'];?></strong></td>
					</tr>
					<tr>
						<td>SP</td><td> : </td><td><strong><?php echo $info['sp'];?></strong></td>
					</tr>
				</table>
			</div><!-- /.col -->
		</div><!-- /.row -->
			...
			<div class="col-xs-7">
				<table class="table table-striped">
					<thead>
						...
						<tr>
							<td>Transportir</td>
							<td><b>: <?php echo $this->supplier_model->NamaTransportir($info['transportir']);?></b></td>
						</tr>
						<tr>
							<td>Kode Vendor Transportir</td>
							<td><b>: <?php echo $this->supplier_model->KodeTransportir($info['transportir']);?></b></td>
						</tr>
						<tr>
							<td>Invoice ditujukan ke</td>
							<td><b>: <u><?php echo $this->setting_model->Nama();?></u> cab. <?php echo $this->pajak_model->KotaCabang($info['wp_id']);?></b></td>
						</tr>
						<tr>
							<td>Storage</td>

...
$this->fpdf->SetTitle("REKAP PENJUALAN Mulai : ".$bln." Sampai : ".$thn);
...
$this->fpdf->Cell(21,0.5,"REKAP PENJUALAN",0,0,'L');
...
$this->fpdf->Cell(3,0.5,"INVOICE NUMBER",0,0,'L');
$this->fpdf->Cell(8,0.5,"CUSTOMER",0,0,'L');
...
$this->fpdf->Cell(28,0.1,"",'B',0,'C');
 ...
 if($isi->num_rows()>0){
		...
		$inv	= $db['jurnal_id']."/INV/".$cab;
		$customer = $this->customer_model->NamaCustomer(($db['customer_id'])); 
		...
		$total = number_format ($db['tot9'], 0, ',', '.'); } 
			...
			$this->fpdf->Cell(1,0.5,"$no",0,0,'L');
			$this->fpdf->Cell(1.5,0.5,"$date",0,0,'L');
			$this->fpdf->Cell(3,0.5,"$inv",0,0,'L');
			$this->fpdf->Cell(8,0.5,$customer,0,0,'L');
...
$this->fpdf->Output("Rekap_Penjualan_$kota.pdf","I");
?> 
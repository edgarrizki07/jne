	...
	<?php }else{ ?>
	<?php $this->db->order_by($order_column='id',$order_type='desc'); $this->db->where('status', $uri3); ?>
	<?php } ?>
	<?php $sat = $this->db->get_where('emailauto',array('cek'=>'1')); $pesan = $sat->result(); ?>
    <?php $no=0; foreach($pesan as $row ): $no++;?>
    <tr>
        <td><?php echo $no;?></td>
		<td><?php echo $row->id;?></br></td>
		<td><?php echo $row->email;?></br></td>
        <td><?php if($row->status=='0'){ ?>Tidak Terkirim<?php }else if($row->status=='1'){ ?>Terkirim<?php }else if($row->status=='2'){ ?>Ganda<?php } ?></td>

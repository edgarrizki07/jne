<div class="content-wrapper">
			...
			<div class="col-xs-12">
				<div class="box">
					<div class="box-header">
						...
						<?php } ?>
							<a target="_blank" class="btn btn-danger" href="<?php echo site_url();?>jual/cancel"> <?php echo $this->bbm_model->JmlJualCancel();?> Cancel</a>
						</div>
					</div><!-- /.box-header -->
					<div class="box-body">
	...
	$t=($row->ohp); $td=($t*$qty); $tt=($td)+($row->ppnohp*$td/10); $gr_total=$disctotal+$ppn+$pbbkb+$pph+($tt); ?>
								<tr>
										...
										<div class="btn-group">
										  ...
										  <ul class="dropdown-menu" role="menu">
											...
											<?php if($row->cek=='0'){ ?>
												...
												<a title="Buat DO" href="<?php echo site_url('jual/add_do/'.$row->id);?>" class="btn btn-success btn-xs"><i class='fa fa-plus'></i> DO</a>
												<?php }else{ ?>
												...
												<?php } ?>
											<?php }else{ ?>
												<a title="lihat DO" href="<?php echo site_url('jual/delivery_order/'.$row->id);?>" class="btn btn-primary btn-xs" ><i class='fa fa-print'></i> DO</a>
											<?php } ?>
									...
									<br><a title="nomor urut penjualan dari masing-masing cabang"><?php echo $row->id_jual;?></a>
									</td>
								<?php if($this->session->userdata('ADMIN') =='1'){ ?>
								...
								<?php } ?>
									...
									<td>Qty : <b class="pull-right"><?php echo number_format ($qty, 0, ',', '.');?></b>
									<br>Delivered : <b class="pull-right"><?php $sell = $this->bbm_model->JmlJual($row->id) ; ?><?php $delivered = $this->bbm_model->DeliveredDO($row->id) ; echo number_format ($delivered, 0, ',', '.');?> L </b>
									<br>Sisa : <b class="pull-right"><?php $sisa = $sell-$delivered ; ?><p class="text-red"><?php echo  number_format ($sisa, 0, ',', '.');?> L</p></b></td>
								</tr>
	<?php endforeach;?>
							</tbody>
						</table>
